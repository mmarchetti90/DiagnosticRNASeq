...
def get_numers(options):
    ...
    for l in input_file:
        if first_line:
            header=(" ".join(l.strip().decode('utf8').split(" ")[1:])+'\n').encode()
            fout.write(header) # print the sample names
            first_line=False
        else:
            words=l.strip().decode('utf8').split(" ")
            new_line=(words[0]+ " "+ " ".join( [ g.split("/")[0] for g in words[1:] ] ) +'\n').encode()
            fout.write(new_line)
