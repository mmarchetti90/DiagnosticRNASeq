...
def call_splicing_outliers_shell(stan_file, out_prefix, cluster_jxn_data_structure, sample_names, num_background_samples, num_simulated_reads, seed, cpus):
	...
	# Load in pystan code
	stan_code = open("./scripts/spot/dirichlet_multinomial.stan").read()

	# Initialize output files
	t_MD = open(out_prefix + '_md.txt','w')  # Filehandle for matrix of mahalanobis distances
	t_pvalue = open(out_prefix + '_empirical_pvalue.txt', 'w')  # Filehandle for matrix of pvalues
	...
	for counter, cluster_id in enumerate(sorted(cluster_jxn_data_structure.keys())):
		...
		try:
			...
			# Print empirical pvalue results to output file
...
if __name__ == "__main__":
	...
	parser.add_option("-t", "--stan", dest="stanfile", default="./scripts/spot/dirichlet_multinomial.stan",
		help="Text file with the stan model")
	...
	parser.add_option("-e", "--numbackgroundsamples", type="int", dest="numbackgroundsamples", default=1000000,
		help="Number of randomly drawn samples per cluster used to generate an empirical mahalanobis distance distribution")
